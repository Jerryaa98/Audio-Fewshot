#!/usr/bin/env python3
"""Qwen2-Audio HELA-VFA episodic few-shot evaluation (frozen backbone).

Adds HELA-VFA (Hellinger-distance prototype classifier) to the Qwen2-Audio
encoder eval pipeline alongside proto/baseline/baseline++/dn4/mcl. Reuses
cache and samplers from ``qwen2audio_episode_eval``; reuses HELA-VFA core
from ``clap_hela_vfa_episode_eval``.
"""
...
import qwen2audio_episode_eval as qee
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Qwen2-Audio HELA-VFA IID/OOD episodic evaluation")
    ...
    parser.add_argument("--finetuned-ckpt", type=str, default=None,
                        help="Optional checkpoint from qwen2audio_hela_vfa_finetune.py / qwen2audio_finetune_lastk.py")

    parser.add_argument("--cache-path", type=str, default="cache/qwen2audio_test_embeddings.npz",
                        help="Shared cache format with qwen2audio_episode_eval.py (backend='qwen2audio')")
    parser.add_argument("--recompute-cache", action="store_true")

    parser.add_argument("--output-csv", type=str, default="results/qwen2audio_hela_vfa_iid_ood_test_eval.csv")
...
def episode_accuracy(
    ...
    if algorithm == "proto" and args.proto_bpa:
        ...
        return qee.proto_accuracy(bpa_features, s_idx, support_y, q_idx, query_y, n_way)
    return qee.episode_accuracy(
...
def run_eval(args: argparse.Namespace) -> None:
    ...
    qee.set_seed(args.episode_seed)
    runtime_device = qee.resolve_runtime_device(args.device)

    data_root = Path(args.data_root)
    split_file = Path(args.split_file)
    sorted_root = Path(args.sorted_root) if args.sorted_root else qee.infer_sorted_root(
        data_root, args.spec_root_name, args.sorted_root_name,
    )

    test_classes = qee.read_test_classes(split_file)
    samples = qee.build_samples(data_root=data_root, sorted_root=sorted_root, test_classes=test_classes)
    class_to_indices, class_bg_groups = qee.build_index(samples)
    ...
    features, map_features = qee.load_or_build_embeddings(
    ...
    for scenario in args.scenarios:
        for algorithm in args.algorithms:
            ...
            for _ in tqdm(range(args.episodes), desc=f"{scenario.upper()} | {algorithm}", leave=True):
                try:
                    if scenario == "iid":
                        s_idx, s_y, q_idx, q_y, chosen = qee.sample_iid_episode(
                            classes_available, class_to_indices, args.n_way, args.k_shot, args.q_query)
                    else:
                        s_idx, s_y, q_idx, q_y, chosen = qee.sample_ood_episode(
            ...
            mean, ci_val = qee.compute_ci95(accs)
            rows.append({
    ...
    print("=== Qwen2Audio HELA-VFA IID/OOD Episodic Results ===")
    for row in rows:
