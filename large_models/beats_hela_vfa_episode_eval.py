#!/usr/bin/env python3
"""BEATs HELA-VFA episodic few-shot evaluation (frozen backbone).

Adds HELA-VFA (Hellinger-distance prototype classifier) to the BEATs eval
pipeline alongside proto/baseline/baseline++/dn4/mcl. Reuses cache and samplers
from ``beats_episode_eval``; reuses HELA-VFA core from
...
import beats_episode_eval as be
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="BEATs HELA-VFA IID/OOD episodic evaluation")
    ...
    parser.add_argument("--beats-ckpt", type=str, default=None,
                        help="Required unless --self-test or cache hit")
    parser.add_argument("--finetuned-ckpt", type=str, default=None,
                        help="Optional checkpoint from beats_hela_vfa_finetune.py / beats_finetune_lastk.py")

    parser.add_argument("--cache-path", type=str, default="cache/beats_test_embeddings.npz",
                        help="Shared cache format with beats_episode_eval.py (backend='beats')")
    parser.add_argument("--recompute-cache", action="store_true")

    parser.add_argument("--output-csv", type=str, default="results/beats_hela_vfa_iid_ood_test_eval.csv")
...
def episode_accuracy(
    ...
    if algorithm == "proto" and args.proto_bpa:
        ...
        return be.proto_accuracy(bpa_features, s_idx, support_y, q_idx, query_y, n_way)
    return be.episode_accuracy(
        algorithm=algorithm, features=features,
        support_idx=support_idx, support_y=support_y,
        query_idx=query_idx, query_y=query_y,
        n_way=n_way, k_shot=k_shot, device=device,
        args=args, map_features=map_features,
    )
...
def run_eval(args: argparse.Namespace) -> None:
    ...
    be.set_seed(args.episode_seed)
    runtime_device = be.resolve_runtime_device(args.device)

    data_root = Path(args.data_root)
    split_file = Path(args.split_file)
    sorted_root = Path(args.sorted_root) if args.sorted_root else be.infer_sorted_root(
        data_root, args.spec_root_name, args.sorted_root_name,
    )

    test_classes = be.read_test_classes(split_file)
    samples = be.build_samples(data_root=data_root, sorted_root=sorted_root, test_classes=test_classes)
    class_to_indices, class_bg_groups = be.build_index(samples)
    ...
    if not args.beats_ckpt and (not cache_path.exists() or args.recompute_cache):
        raise ValueError("--beats-ckpt is required when no valid embedding cache exists")

    features, map_features = be.load_or_build_embeddings(
        samples=samples, cache_path=cache_path, recompute=args.recompute_cache,
        beats_ckpt=args.beats_ckpt or "",
    ...
    for scenario in args.scenarios:
        for algorithm in args.algorithms:
            ...
            for _ in tqdm(range(args.episodes), desc=f"{scenario.upper()} | {algorithm}", leave=True):
                try:
                    if scenario == "iid":
                        s_idx, s_y, q_idx, q_y, chosen = be.sample_iid_episode(
                            classes_available, class_to_indices, args.n_way, args.k_shot, args.q_query)
                    else:
                        s_idx, s_y, q_idx, q_y, chosen = be.sample_ood_episode(
            ...
            mean, ci_val = be.compute_ci95(accs)
            rows.append({
    ...
    print("=== BEATs HELA-VFA IID/OOD Episodic Results ===")
    for row in rows:
