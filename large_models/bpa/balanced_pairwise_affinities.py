...
class BPA(nn.Module):
    ...
    def __init__(self,
        ...
        self.diagonal_val = 1e5   
        self.labels = torch.cat([
                torch.full((num_way * num_query,), -1, dtype=torch.long),
                torch.tensor([i % num_way for i in range(num_shot * num_way)], dtype=torch.long)
            ])
    ...
    def forward(self, x: Tensor, y: Tensor = None, labels: Tensor = None) -> Tensor:
        """Compute the BPA feature transform with fixed blending"""
        # Compute  metrics
