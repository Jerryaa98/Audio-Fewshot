...
import numpy as np
from libfewshot_core.utils import accuracy, majority_vote, vote_catagorical_acc, average_logits
...
class DeepBDC(MetricModel):
    ...
    def set_forward(self, batch, update_threshold=False, enhance_classification_via_energy=False):
        if len(batch) == 2:
        ...
        if False:  # enable BPA
            feat = BPA(distance_metric='cosine')(feat)
        ...
        avg_logits = average_logits(output, repeats)
        uncertains = -torch.logsumexp(avg_logits, dim=1)
        if update_threshold:
            is_correct = (pre_query_pred.reshape(-1).detach().cpu().numpy() == post_query_y.detach().cpu().numpy())
            self.uncertainty_threshold.append([uncertains.detach().cpu().numpy(), is_correct])
        
        if not enhance_classification_via_energy:
            return output, acc
        else:
            # print(uncertains, self.uncertain_global_threshold)
            fname = 'test_uncertainty.npy'
            try:
                cur = uncertains.detach().cpu().numpy().ravel()
                if os.path.exists(fname):
                    prev = np.load(fname)
                    prev = np.asarray(prev).ravel()
                    np.save(fname, np.concatenate([prev, cur]))
                else:
                    np.save(fname, cur)
                # print(f"Saved uncertainties to {fname}")
            except Exception as e:
                print(f"Could not save uncertainties to {fname}: {e}")
            if False:
                uncertains = (uncertains - self.uncertains_mean) / self.uncertains_std
            
            ood_query_mask = np.zeros(uncertains.shape[0], dtype=bool)
            ood_query_mask[np.argsort(-uncertains.detach().cpu().numpy())[:int(0.2*len(uncertains))]] = True
            # ood_query_mask = (uncertains.detach().cpu().numpy() > self.uncertain_global_threshold)
            
            
            # ood_query_idxs = np.where(uncertains.detach().cpu().numpy() < self.uncertain_global_threshold)[0]
            # query_idxs = np.where(query_mask)[0]
            # query_mask[query_idxs[ood_query_idxs]] = False
            return output, acc, uncertains, ood_query_mask, query_mask
    ...
    def get_uncertainty_threshold(self, policy='mean', normalize=False):
        if len(self.uncertainty_threshold) == 0:
            return None, None
        uncertains = []
        is_corrects = []

        if policy == 'overall':
            for item in self.uncertainty_threshold:
                uncertains.extend(item[0].tolist())
                is_corrects.extend(item[1].tolist())
            uncertains = np.array(uncertains)
            is_corrects = np.array(is_corrects)
            
            self.uncertains_mean = np.mean(uncertains[is_corrects==True])
            self.uncertains_std = np.std(uncertains[is_corrects==True])
            if normalize:
                uncertains[is_corrects==True] = (uncertains[is_corrects==True] - self.uncertains_mean) / self.uncertains_std
            is_corrects = np.array(is_corrects)

            correct_uncertains = uncertains[is_corrects==True]

            self.uncertain_global_threshold = np.quantile(correct_uncertains, 0.95)
        else:
            for item in self.uncertainty_threshold:
                uncertains.append(item[0])
                is_corrects.append(item[1])
            
            for u in uncertains:
                u = np.array(u)
                self.uncertains_mean = []
                self.uncertains_std = []
                if normalize:
                    # if not hasattr(self, 'uncertains_mean'):
                    self.uncertains_mean.append(np.mean(u))
                    self.uncertains_std.append(np.std(u))
            
            self.uncertains_mean = np.mean(self.uncertains_mean)
            self.uncertains_std = np.mean(self.uncertains_std)
            
            # save npz file
            np.savez('uncertainty_data.npz', uncertains=uncertains, is_corrects=is_corrects)
            print('saved uncertainty data to uncertainty_data.npz')

            thresholds = []
            for u, ic in zip(uncertains, is_corrects):
                u = np.array(u)
                ic = np.array(ic)
                correct_uncertains = u[ic==True]
                if len(correct_uncertains) == 0:
                    continue
                
                if normalize and False:
                    correct_uncertains = (correct_uncertains - self.uncertains_mean) / self.uncertains_std
                th = np.quantile(correct_uncertains, 0.95)
                thresholds.append(th)
            # print('thresholds are:{}'.format(thresholds))
            # print('mean of thresholds is:{}'.format(np.mean(thresholds)))
            # print('std of thresholds is:{}'.format(np.std(thresholds)))
            # print('uncertains mean is:{}'.format(self.uncertains_mean))
            # print('uncertains std is:{}'.format(self.uncertains_std))
            # input()
            self.uncertain_global_threshold = np.mean(thresholds)