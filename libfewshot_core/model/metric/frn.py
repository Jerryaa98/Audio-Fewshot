...
class FRNLayer(nn.Module):
    ...
    def get_neg_l2_dist(self, support, query, way, shot, query_shot):
        ...
        neg_l2_dist = (
            recon_dist.neg().view(-1, resolution, way).mean(1)
        )  # way*query_shot, way
...
class FRN(MetricModel):
    ...
    def set_forward(self, batch):
        ...
        for i in range(len(query_feat)):
            output.append(self.frn_layer(support_feat, query_feat[i].unsqueeze(0).permute(0, 1, 3, 4, 2).contiguous().view(episode_size, -1, c)
            , self.way_num, self.shot_num, self.query_num).reshape(-1, self.way_num))
        ...
        post_query_y = torch.repeat_interleave(query_target.reshape(-1).to('cuda'), repeats.to('cuda')).to('cuda', dtype=torch.long)
