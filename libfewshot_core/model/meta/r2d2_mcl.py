...
from libfewshot_core.model.metric.mcl import MCLMask
...
class R2D2MCL(MetaModel):
    def __init__(self, **kwargs):
        super(R2D2MCL, self).__init__(**kwargs)
    ...
    def set_forward(self, batch):
        ...
        query_mel = self.mel_mask(support_feat, query_feat, self.n_way, self.k_shot)
        b, q, c, h, w = query_feat.shape
        s = support_feat.shape[1]
        query_feat = (query_feat * query_mel).view(b, q, c, h * w).sum(-1)
        support_feat = F.adaptive_avg_pool2d(support_feat.view(-1, c, h, w), 1).view(b, s, c)
