...
class Trainer(object):
    ...
    def __init__(self, rank, config):
        ...
        self.is_clap = config.get("is_clap", False)
        ...
        # Import numba BEFORE initializing logger to avoid module resolution conflicts
        # numba must see the original builtins.print, not our custom logger
        # import numba
        ...
        with torch.set_grad_enabled(enable_grad):
            ...
            for batch_idx, batch in enumerate(zip(*loader)):
                ...
                output, acc = self.model(
                    [(elem.to('cuda') if not isinstance(elem, int) and elem is not None else elem) for each_batch in batch for elem in each_batch]
                )
                meter.update("calc_time", time() - calc_begin)
    ...
    def _init_logger(self):
        ...
        # hack print
        def use_logger(
            *msg, level="info", all_rank=False, sep=' ', end='\n', file=None, flush=False
    ...
    def _init_model(self, config):
        ...
        model_kwargs = {
            ...
            "is_clap": config["is_clap"] if "is_clap" in config else False,
        }
        ...
        if self.is_clap:
            print("Loading CLAP pretrained weights...")
            # model.emb_func._load_finetuned_weights('/root/CLAP-OOD/clap_episodic/clap_episodic_epoch15.pt')
