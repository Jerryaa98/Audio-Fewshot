...
class FewShotAugCollateFunctionAudio(object):
    ...
    def __init__(self, trfms, times, times_q, way_num, shot_num, query_num, mode='train', is_clap=False):
        """Initialize a `FewShotAugCollateFunction`.
    ...
    def method(self, batch):
        ...
        try:
            ...
            support_counter = 0
            # helper_counter = 0
            if self.mode != 'train' and not self.is_clap:
                # if self.mode == 'test':
                #     print("in test mode")
                audios_split_by_label_type = []
            ...
            if self.mode == 'train' or self.is_clap:
                # repeats_ = None
                repeats_ = torch.ones(self.query_num * self.way_num, dtype=torch.int64)
