...
from libfewshot_core.utils import SaveType
...
def average_logits(soft_logits, query_nums):
    """
    Aggregate repeated logits by averaging across repeats for each query group.

    Args:
        soft_logits (torch.Tensor): Tensor of shape (total_repeats, num_classes).
        query_nums (iterable[int]): Number of repeats per query (length = num_queries).

    Returns:
        torch.Tensor: Tensor of shape (num_queries, num_classes) with averaged logits.
    """
    ...
    for num in query_nums:
        if num == 0:
            averaged.append(torch.zeros(soft_logits.size(1), device=device, dtype=soft_logits.dtype))
            continue
        slice_logits = soft_logits[start:start + num]
        avg = slice_logits.mean(dim=0)
        averaged.append(avg)
        start += num
