...
class SwinTransformer(nn.Module):
    ...
    def forward(self, img):
        x = self.stage1(img)  # 224,224 -> 56,56
        x = self.stage2(x)  # 56,56 -> 28,28
        x = self.stage3(x)  # 28,28 -> 14,14
        x = self.stage4(x)  # 4,14 -> 7,7
        if self.pool:
            x = x.mean(dim=[2, 3])
        else:
...
if __name__ == "__main__":
    ...
    input_x = torch.randn((10, 3, 84, 84))
